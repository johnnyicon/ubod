...
**Without Orchestration (Monolithic):**
```
┌─────────────────────────────────────────────┐
│         Single Agent (Overloaded)          │
│                                             │
│  Does: Discovery + Design + Verification   │
│  Risk: Misses patterns, makes assumptions  │
│  Output: Looks complete, but full of gaps  │
└─────────────────────────────────────────────┘
```

**With Orchestration (Specialized):**
```
┌───────────────────────────────────────────────┐
│           Orchestrator (Coordinator)          │
│  Plans workflow → Delegates → Merges output  │
└───────────────────────────────────────────────┘
         │
         ├─────────────────┬─────────────────┐
         ↓                 ↓                 ↓
...
│    Planner      │ │   Designer  │ │               │
│                 │ │             │ │               │
│ Maps patterns   │ │ Designs UI  │ │ Checks gaps   │
│ Returns proof   │ │ Plans QA    │ │ Validates all │
└─────────────────┘ └─────────────┘ └───────────────┘
```
...
**Anti-patterns — Don't orchestrate when:**

| Scenario | Why Not to Orchestrate | Better Approach |
|----------|------------------------|------------------|
| **Simple CRUD operation** | Adding a model field is straightforward | Single agent or prompt |
| **Single-file changes** | No cross-domain complexity | Direct implementation |
| **Well-documented task** | Clear spec, no discovery needed | Follow existing pattern |
| **Tight latency budget** | Orchestration adds overhead (multiple agent activations) | Single optimized agent |
| **Linear task (no dependencies)** | No benefit from specialist separation | Single agent processes sequentially |

**Red flags indicating over-orchestration:**
- "I need an orchestrator to call one specialist" → Just use the specialist
- "Each specialist does < 30 seconds of work" → Coordination overhead too high
- "Specialists don't share context or dependencies" → No workflow benefit
- "I'm orchestrating just to use the pattern" → Pattern for pattern's sake

**Rule of thumb:** Orchestrate when coordination saves more effort than it costs.
...
**Can sub-agents invoke sub-sub-agents?**

Technically yes, but **depth limits are critical:**

| Depth | Pattern | Status |
|-------|---------|--------|
| **Depth 0** | No orchestration (single agent) | ✅ Standard |
| **Depth 1** | Orchestrator → Specialists | ✅ Recommended |
| **Depth 2** | Orchestrator → Sub-orchestrator → Specialists | ⚠️ Complex |
| **Depth 3+** | Multiple nested levels | 🚫 Avoid |

**Why limit depth?**

1. **Debugging complexity:** Each level multiplies troubleshooting difficulty
2. **Latency accumulation:** Depth 3 = 8+ agent activations = slow
3. **Context loss:** Information degrades as it passes through layers
4. **Cognitive overhead:** Hard to trace what happened where

**When depth 2 is acceptable:**
- Sub-orchestrator encapsulates a well-defined sub-workflow
- Sub-workflow is reusable across multiple top-level orchestrations
- Benefits of encapsulation outweigh complexity cost

**Example (acceptable depth 2):**
```
PRD Enricher (Orchestrator)
    ↓
    ├─→ Discovery Planner (Specialist)
    ├─→ UI/UX Designer (Specialist)
    └─→ Testing Strategy Designer (Sub-orchestrator)
            ↓
...
**Why this works:** Testing Strategy Designer is a reusable sub-workflow used by multiple parent orchestrators.

**Best practice:** Start with depth 1. Only add depth 2 when sub-workflow reuse is proven valuable.
...
**Orchestration has overhead. Optimize when:**

#### 1. Latency Budget is Tight

**Problem:** Each `runSubagent` call has activation overhead:
- Agent load time: ~1-2 seconds
- Context parsing: ~0.5-1 second
- Response generation: Depends on task

**3 specialist agents = 6-9 seconds overhead minimum**

**Mitigation strategies:**
- Combine specialists when dependencies allow
- Use caching for repeated orchestrations
- Front-load expensive operations (discovery before design)
- Consider single optimized agent for latency-critical paths

#### 2. Token Budget Concerns

**Problem:** Orchestration uses more tokens:
- Orchestrator context
- Each specialist's context
- Handoff announcements
- Merged output

**Example token usage:**
```
Single agent:     5K tokens
Orchestrated:    15K tokens (3 specialists × 5K each)
```

**Mitigation strategies:**
- Efficient specialist prompts (clear, focused tasks)
- Structured output formats (easier to parse = less repetition)
- Selective handoffs (only invoke specialists when needed)

#### 3. Monitoring Orchestration Health

**Track these metrics:**
- **Specialist invocation count** — Are all specialists necessary?
- **End-to-end latency** — Is orchestration slower than acceptable?
- **Output quality** — Is orchestration producing better results?
- **Specialist reuse** — Are specialists used in multiple orchestrations?

**Red flags:**
- Orchestration takes > 2× longer than single agent but quality is similar
- Specialists are invoked but output is ignored
- Same orchestration pattern isn't reused elsewhere

**Green flags:**
- Output quality significantly better with orchestration
- Specialists are reused across multiple orchestrations
- Debugging is easier due to specialist separation
